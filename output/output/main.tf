...
resource "google_monitoring_alert_policy" "audit_log_changes" {
  display_name = "Audit Log Configuration Changes"
  project      = var.project_id
  combiner     = "OR"

  conditions {
    display_name = "Audit Log Changes"
    condition_threshold {
      filter          = "resource.type = gcp_project AND log_name = \"projects/${var.project_id}/logs/cloudaudit.googleapis.com%2Fsystem_event\" AND protoPayload.methodName = \"SetIamPolicy\""
      duration        = "60s"
